		...
		do {
			...
			if(vip == 1) {
				...
				if(f.getCreditos() >= f.vireSocio(vip)) {
					f.setCreditos( f.getCreditos() - f.vireSocio(vip));
					System.out.println("Compra Realizada com Sucesso!"
							+ "\n Seu Saldo atual e aproximadamente de "+f.getCreditos());
					System.out.print("\n----------------------------------------");
				}else {
					System.out.println(" não há credito!");
					break;
				}
				...
				if(f.getCreditos() >= f.vireSocio(vip)) {
					...
					System.out.println(" nao ha creditos!");
					break;
				}
